...
int input_init(input_parameter *param)
{
	...
	if( pthread_mutex_init(&controls_mutex, NULL) != 0 )		// 初始化 controls_mutex
	{
	...
	videoIn = malloc(sizeof(struct vdIn));	// 分配一个 vdIn 给够体
	if ( videoIn == NULL )
	{
		IPRINT("not enough memory for videoIn\n");
		exit(EXIT_FAILURE);
	}
	memset(videoIn, 0, sizeof(struct vdIn));	// 将该机构体清为0
	...
	*/
	while( !pglobal->stop )
	{
		...
		*/
		...
		if ( videoIn->fps < 5 )		// 如果我们的帧率小于5,则要做一个小的延时操作
		{
